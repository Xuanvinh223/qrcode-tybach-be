...
type PurdService struct {
	*BaseService
	*CommonFunction
}

var Purd = &PurdService{}

func (s *PurdService) GetPurListD2(requestParams request.PurListRequestKHPO) ([]types.PurList_Nosize_KHPO, error) {
	...
	if err != nil {
		return []types.PurList_Nosize_KHPO{}, err
	}
	dbInstance, _ := db.DB()

	query := fmt.Sprintf(`
		select cgzlss.CGNO, DDZL.KHPO, DDZL.ARTICLE, cgzlss.CLBH, CLZL.YWPM, CONVERT(varchar, CGZLSS.YQDate, 111) as YQDate, ZSZL.ZSDH, zszl.zsywjc,isnull(KCRKScan_RFSS.PackQty,0) as PackQty,
				sum(cgzlss.qty) as Qty,isnull(KCRKScan_RFSS.qty,0) as DelQty, cgzlss.ZLBH
		from cgzlss left join cgzl on cgzl.cgno=cgzlss.cgno left join ZSZL on CGZL.ZSBH = ZSZL.ZSDH 
		left join CLZL on cgzlss.CLBH = CLZL.CLDH 
		left join (Select KCRKScan_RFS.CLBH,KCRKScan_RFS.CGNO, KCRKScan_RFSS.Memo_RY ZLBH, SUM(KCRKScan_RFSS.qty) as qty, max(KCRKScan_RFSS.CFMDel) as CFMDel, pack.PackQty
		...
		left join( select KCRKScan_RFS.CLBH,KCRKScan_RFS.CGNO, KCRKScan_RFSS.Memo_RY ZLBH,max(isnull(KCRKScan_RFS.PackQty,0)) as PackQty
		from KCRKScan_RFS
		...
		group by KCRKScan_RFS.CLBH,KCRKScan_RFS.CGNO,KCRKScan_RFSS.Memo_RY)pack on pack.CGNO=KCRKScan_RFS.CGNO and pack.CLBH=KCRKScan_RFS.CLBH and pack.ZLBH=KCRKScan_RFSS.Memo_RY
		where KCRKScan_RF.LB='02'
			group by KCRKScan_RFS.CLBH,KCRKScan_RFS.CGNO,KCRKScan_RFSS.Memo_RY,pack.PackQty) 
		KCRKScan_RFSS on KCRKScan_RFSS.CGNO=cgzlss.cgno and KCRKScan_RFSS.CLBH=cgzlss.CLBH and KCRKScan_RFSS.ZLBH=cgzlss.ZLBH 
		...
		and DDZL.KHPO like '%%%s%%' and DDZL.ARTICLE like '%%%s%%' and CGZLSS.ZLBH like '%%%s%%' and CLZL.YWPM like '%%%s%%'
		Group by cgzlss.CGNO, DDZL.KHPO, DDZL.ARTICLE,cgzlss.CLBH,CLZL.YWPM,cgzlss.ZLBH,CONVERT(varchar, CGZLSS.YQDate, 111),ZSZL.ZSDH,zszl.zsywjc,KCRKScan_RFSS.PackQty,KCRKScan_RFSS.qty,KCRKScan_RFSS.CFMDel
		...
		order by YQDate desc, cgzlss.CGNO, cgzlss.CLBH,cgzlss.ZLBH asc
	`,
		requestParams.ZSDH, requestParams.YQDate1, requestParams.YQDate2, requestParams.KHPO, requestParams.ARTICLE, requestParams.ZLBH, requestParams.YWPM)

	var result []types.PurList_Nosize_KHPO
...
func (s *PurdService) SetPackQtyD2(requestParams request.PackQtyRequestS) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) GetDetailListD2(requestParams request.DetailListRequestS) ([]types.DetailListS, error) {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) AddPackD2(SCNO string, CLBH string, ZLBH string, USERID string, QTYAD int) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) DelPackD2(SCNO string, CLBH string, USERID string, QTYAD int) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) LoadingQtyD2(requestParams request.LoadRequestS) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) UploadLotFileD2(requestParams request.UploadLotFileRequest, LotFile *multipart.FileHeader) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
...
func (s *PurdService) CFMD2(requestParams request.CFMRequest) error {
	configuration := config.GetConfig()
	configuration.Database.Driver = "sqlserver"
	configuration.Database.Host = "192.168.40.9"
	configuration.Database.Username = "tyxuan"
